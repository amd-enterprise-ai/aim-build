...
This guide provides step-by-step instructions for deploying AMD Inference Microservice (AIM) container for
 google/gemma-4-31B-it model in
...
docker run \
  ...
  -p 8000:8000 \
  amdenterpriseai/aim-google-gemma-4-31b-it:0.13.0-preview
```
...
docker run --rm \
  ...
  amdenterpriseai/aim-google-gemma-4-31b-it:0.13.0-preview \
  download-to-cache --model-id google/gemma-4-31B-it
```
...
spec:
  ...
  template:
    ...
    spec:
      containers:
        - name: minimal-aim-deployment
          image: amdenterpriseai/aim-google-gemma-4-31b-it:0.13.0-preview
