...
This guide provides step-by-step instructions for deploying AMD Inference Microservice (AIM) container for
 google/gemma-3-27b-it model in
...
docker run \
  ...
  -p 8000:8000 \
  amdenterpriseai/aim-google-gemma-3-27b-it:0.11.1
```
...
docker run --rm \
  ...
  amdenterpriseai/aim-google-gemma-3-27b-it:0.11.1 \
  download-to-cache --model-id google/gemma-3-27b-it
```
...
spec:
  ...
  template:
    ...
    spec:
      containers:
        - name: minimal-aim-deployment
          image: amdenterpriseai/aim-google-gemma-3-27b-it:0.11.1
