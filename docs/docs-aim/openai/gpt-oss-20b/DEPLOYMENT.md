...
docker run \
  ...
  -p 8000:8000 \
  amdenterpriseai/aim-openai-gpt-oss-20b:0.10.0
```
...
docker run --rm \
  ...
  amdenterpriseai/aim-openai-gpt-oss-20b:0.10.0 \
  download-to-cache --model-id openai/gpt-oss-20b
```
...
```bash
docker run \
  -e AIM_PROFILE_ID=vllm-mi250x-mxfp4-tp1-latency \
  --device=/dev/kfd --device=/dev/dri \
  -p 8000:8000 \
  amdenterpriseai/aim-openai-gpt-oss-20b:0.10.0
```
