...
docker run \
  ...
  -p 8000:8000 \
  amdenterpriseai/aim-epyc-google-gemma-3-4b-it:0.13.0
```
...
docker run --rm \
  ...
  amdenterpriseai/aim-epyc-google-gemma-3-4b-it:0.13.0 \
  download-to-cache --model-id google/gemma-3-4b-it
```
...
spec:
  ...
  template:
    ...
    spec:
      containers:
        - name: minimal-aim-deployment
          image: amdenterpriseai/aim-epyc-google-gemma-3-4b-it:0.13.0
