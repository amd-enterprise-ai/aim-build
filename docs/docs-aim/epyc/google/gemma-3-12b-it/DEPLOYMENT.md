...
This guide provides step-by-step instructions for deploying AMD Inference Microservice (AIM) container for
 google/gemma-3-12b-it model in
...
docker run \
  ...
  -p 8000:8000 \
  amdenterpriseai/aim-epyc-google-gemma-3-12b-it:0.13.0
```
...
docker run --rm \
  ...
  amdenterpriseai/aim-epyc-google-gemma-3-12b-it:0.13.0 \
  download-to-cache --model-id google/gemma-3-12b-it
```
...
spec:
  ...
  template:
    ...
    spec:
      containers:
        - name: minimal-aim-deployment
          image: amdenterpriseai/aim-epyc-google-gemma-3-12b-it:0.13.0
