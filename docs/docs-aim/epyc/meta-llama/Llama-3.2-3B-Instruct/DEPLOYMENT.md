...
This guide provides step-by-step instructions for deploying AMD Inference Microservice (AIM) container for
 meta-llama/Llama-3.2-3B-Instruct model in
...
docker run \
  ...
  -p 8000:8000 \
  amdenterpriseai/aim-epyc-meta-llama-llama-3-2-3b-instruct:0.11.0-preview
```
...
docker run --rm \
  ...
  amdenterpriseai/aim-epyc-meta-llama-llama-3-2-3b-instruct:0.11.0-preview \
  download-to-cache --model-id meta-llama/Llama-3.2-3B-Instruct
```
...
aim_id: meta-llama/Llama-3.2-3B-Instruct
...
docker run \
  -e HF_TOKEN=<YOUR_HUGGINGFACE_TOKEN> \
  -e AIM_PROFILE_ID=vllm-epyc_zen4-bf16-tp1-latency \
  -p 8000:8000 \
  amdenterpriseai/aim-epyc-meta-llama-llama-3-2-3b-instruct:0.11.0-preview
```
