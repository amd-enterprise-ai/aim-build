...
"""
Tests for the per-engine Pydantic argument models (now homed in each engine
module and re-exported from ``aim_runtime.engines``).
"""
...
class TestVllmOmniEngineArgsModel:
    ...
    def test_is_vllm_subclass(self):
        assert issubclass(VllmOmniEngineArgsModel, VllmEngineArgsModel)
