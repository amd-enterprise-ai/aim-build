...
This guide provides step-by-step instructions for deploying AMD Inference Microservice (AIM) container for
 deepseek-ai/DeepSeek-R1 model in
...
docker run \
  ...
  -p 8000:8000 \
  amdenterpriseai/aim-deepseek-ai-deepseek-r1:0.11.1
```
...
docker run --rm \
  ...
  amdenterpriseai/aim-deepseek-ai-deepseek-r1:0.11.1 \
  download-to-cache --model-id deepseek-ai/DeepSeek-R1
```
...
spec:
  ...
  template:
    ...
    spec:
      containers:
        - name: minimal-aim-deployment
          image: amdenterpriseai/aim-deepseek-ai-deepseek-r1:0.11.1
...
aim_id: deepseek-ai/DeepSeek-R1
