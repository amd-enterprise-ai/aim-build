...
This guide provides step-by-step instructions for deploying AMD Inference Microservice (AIM) container for
 Qwen/Qwen2.5-Coder-7B-Instruct model in
...
docker run \
  -p 8000:8000 \
  amdenterpriseai/aim-epyc-qwen-qwen2-5-coder-7b-instruct:0.13.0
```
...
docker run --rm \
  ...
  amdenterpriseai/aim-epyc-qwen-qwen2-5-coder-7b-instruct:0.13.0 \
  download-to-cache --model-id Qwen/Qwen2.5-Coder-7B-Instruct
```
...
spec:
  ...
  template:
    ...
    spec:
      containers:
        - name: minimal-aim-deployment
          image: amdenterpriseai/aim-epyc-qwen-qwen2-5-coder-7b-instruct:0.13.0
...
aim_id: Qwen/Qwen2.5-Coder-7B-Instruct
