...
This guide provides step-by-step instructions for deploying AMD Inference Microservice (AIM) container for
 google/medgemma-27b-it model in
...
docker run \
  ...
  -p 8000:8000 \
  amdenterpriseai/aim-google-medgemma-27b-it:0.13.0
```
...
docker run --rm \
  ...
  amdenterpriseai/aim-google-medgemma-27b-it:0.13.0 \
  download-to-cache --model-id google/medgemma-27b-it
```
...
spec:
  ...
  template:
    ...
    spec:
      containers:
        - name: minimal-aim-deployment
          image: amdenterpriseai/aim-google-medgemma-27b-it:0.13.0
