...
This guide provides step-by-step instructions for deploying AMD Inference Microservice (AIM) container for
 meta-llama/Llama-3.1-405B-Instruct model in
...
docker run \
  ...
  -p 8000:8000 \
  amdenterpriseai/aim-meta-llama-llama-3-1-405b-instruct:0.11.1
```
...
docker run --rm \
  ...
  amdenterpriseai/aim-meta-llama-llama-3-1-405b-instruct:0.11.1 \
  download-to-cache --model-id meta-llama/Llama-3.1-405B-Instruct
```
...
spec:
  ...
  template:
    ...
    spec:
      containers:
        - name: minimal-aim-deployment
          image: amdenterpriseai/aim-meta-llama-llama-3-1-405b-instruct:0.11.1
...
aim_id: meta-llama/Llama-3.1-405B-Instruct
...
docker run \
  -e HF_TOKEN=<YOUR_HUGGINGFACE_TOKEN> \
  -e AIM_PROFILE_ID=vllm-mi325x-fp16-tp8-latency \
  --device=/dev/kfd --device=/dev/dri \
  -p 8000:8000 \
  amdenterpriseai/aim-meta-llama-llama-3-1-405b-instruct:0.11.1
```
