...
docker run \
  ...
  -p 8000:8000 \
  amdenterpriseai/aim-meta-llama-llama-3-2-3b-instruct:0.11.1
```
...
spec:
  ...
  template:
    ...
    spec:
      containers:
        - name: minimal-aim-deployment
          image: amdenterpriseai/aim-meta-llama-llama-3-2-3b-instruct:0.11.1
...
docker run \
  -e HF_TOKEN=<YOUR_HUGGINGFACE_TOKEN> \
  -e AIM_PROFILE_ID=vllm-mi300x-fp8-tp1-throughput \
  --device=/dev/kfd --device=/dev/dri \
  -p 8000:8000 \
  amdenterpriseai/aim-meta-llama-llama-3-2-3b-instruct:0.11.1
```
