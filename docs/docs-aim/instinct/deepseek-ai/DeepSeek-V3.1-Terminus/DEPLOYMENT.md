...
This guide provides step-by-step instructions for deploying AMD Inference Microservice (AIM) container for
 deepseek-ai/DeepSeek-V3.1-Terminus model in
...
docker run \
  ...
  -p 8000:8000 \
  amdenterpriseai/aim-deepseek-ai-deepseek-v3-1-terminus:0.11.1
```
...
docker run --rm \
  ...
  amdenterpriseai/aim-deepseek-ai-deepseek-v3-1-terminus:0.11.1 \
  download-to-cache --model-id deepseek-ai/DeepSeek-V3.1-Terminus
```
...
spec:
  ...
  template:
    ...
    spec:
      containers:
        - name: minimal-aim-deployment
          image: amdenterpriseai/aim-deepseek-ai-deepseek-v3-1-terminus:0.11.1
...
aim_id: deepseek-ai/DeepSeek-V3.1-Terminus
