...
This guide provides step-by-step instructions for deploying AMD Inference Microservice (AIM) container for
 Qwen/Qwen3-Coder-Next model in
...
docker run \
  ...
  -p 8000:8000 \
  amdenterpriseai/aim-qwen-qwen3-coder-next:0.11.1
```
...
docker run --rm \
  ...
  amdenterpriseai/aim-qwen-qwen3-coder-next:0.11.1 \
  download-to-cache --model-id Qwen/Qwen3-Coder-Next
```
...
spec:
  ...
  template:
    ...
    spec:
      containers:
        - name: minimal-aim-deployment
          image: amdenterpriseai/aim-qwen-qwen3-coder-next:0.11.1
...
```bash
docker run \
  -e AIM_PROFILE_ID=vllm-mi300x-bf16-tp1-throughput \
  --device=/dev/kfd --device=/dev/dri \
  -p 8000:8000 \
  amdenterpriseai/aim-qwen-qwen3-coder-next:0.11.1
```
