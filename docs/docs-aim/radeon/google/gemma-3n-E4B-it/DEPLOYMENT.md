...
This guide provides step-by-step instructions for deploying AMD Inference Microservice (AIM) container for
 google/gemma-3n-E4B-it model in
...
docker run \
  ...
  -p 8000:8000 \
  amdenterpriseai/aim-radeon-google-gemma-3n-e4b-it:0.12.0-preview
```
...
docker run --rm \
  ...
  amdenterpriseai/aim-radeon-google-gemma-3n-e4b-it:0.12.0-preview \
  download-to-cache --model-id google/gemma-3n-E4B-it
```
