...
docker run \
  ...
  -p 8000:8000 \
  amdenterpriseai/aim-radeon-zai-org-glm-4-7-flash:0.12.0-preview
```
...
docker run --rm \
  ...
  amdenterpriseai/aim-radeon-zai-org-glm-4-7-flash:0.12.0-preview \
  download-to-cache --model-id zai-org/GLM-4.7-Flash
```
...
spec:
  ...
  template:
    ...
    spec:
      containers:
        - name: minimal-aim-deployment
          image: amdenterpriseai/aim-radeon-zai-org-glm-4-7-flash:0.12.0-preview
...
```bash
docker run \
  -e AIM_PROFILE_ID=vllm-r9700-bf16-tp4-latency \
  --device=/dev/kfd --device=/dev/dri \
  -p 8000:8000 \
  amdenterpriseai/aim-radeon-zai-org-glm-4-7-flash:0.12.0-preview
```
