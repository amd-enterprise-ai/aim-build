...
This guide provides step-by-step instructions for deploying AMD Inference Microservice (AIM) container for
 Qwen/Qwen2.5-VL-7B-Instruct model in
...
docker run \
  -p 8000:8000 \
  amdenterpriseai/aim-epyc-qwen-qwen2-5-vl-7b-instruct:0.13.0-preview
```
...
docker run --rm \
  ...
  amdenterpriseai/aim-epyc-qwen-qwen2-5-vl-7b-instruct:0.13.0-preview \
  download-to-cache --model-id Qwen/Qwen2.5-VL-7B-Instruct
```
...
spec:
  ...
  template:
    ...
    spec:
      containers:
        - name: minimal-aim-deployment
          image: amdenterpriseai/aim-epyc-qwen-qwen2-5-vl-7b-instruct:0.13.0-preview
...
aim_id: Qwen/Qwen2.5-VL-7B-Instruct
...
```bash
docker run \
  -e AIM_PROFILE_ID=vllm-epyc_9965-bf16-tp1-latency \
  -p 8000:8000 \
  amdenterpriseai/aim-epyc-qwen-qwen2-5-vl-7b-instruct:0.13.0-preview
```
