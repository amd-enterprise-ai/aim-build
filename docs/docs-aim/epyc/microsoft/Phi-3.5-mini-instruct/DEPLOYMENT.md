...
This guide provides step-by-step instructions for deploying AMD Inference Microservice (AIM) container for
 microsoft/Phi-3.5-mini-instruct model in
...
docker run \
  -p 8000:8000 \
  amdenterpriseai/aim-epyc-microsoft-phi-3-5-mini-instruct:0.13.0
```
...
docker run --rm \
  ...
  amdenterpriseai/aim-epyc-microsoft-phi-3-5-mini-instruct:0.13.0 \
  download-to-cache --model-id microsoft/Phi-3.5-mini-instruct
```
...
spec:
  ...
  template:
    ...
    spec:
      containers:
        - name: minimal-aim-deployment
          image: amdenterpriseai/aim-epyc-microsoft-phi-3-5-mini-instruct:0.13.0
...
aim_id: microsoft/Phi-3.5-mini-instruct
