...
docker run \
  ...
  -p 8000:8000 \
  amdenterpriseai/aim-minimaxai-minimax-m3:2026.9.0
```
...
docker run --rm \
  ...
  amdenterpriseai/aim-minimaxai-minimax-m3:2026.9.0 \
  download-to-cache --model-id MiniMaxAI/MiniMax-M3
```
...
spec:
  ...
  template:
    ...
    spec:
      containers:
        - name: minimal-aim-deployment
          image: amdenterpriseai/aim-minimaxai-minimax-m3:2026.9.0
...
```bash
docker run \
  -e AIM_PROFILE_ID=vllm-mi300x-fp8-tp8-latency \
  --device=/dev/kfd --device=/dev/dri \
  -p 8000:8000 \
  amdenterpriseai/aim-minimaxai-minimax-m3:2026.9.0
```
