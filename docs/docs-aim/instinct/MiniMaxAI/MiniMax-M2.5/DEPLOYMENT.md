...
This guide provides step-by-step instructions for deploying AMD Inference Microservice (AIM) container for
 MiniMaxAI/MiniMax-M2.5 model in
...
docker run \
  ...
  -p 8000:8000 \
  amdenterpriseai/aim-minimaxai-minimax-m2-5:0.12.0
```
...
docker run --rm \
  ...
  amdenterpriseai/aim-minimaxai-minimax-m2-5:0.12.0 \
  download-to-cache --model-id MiniMaxAI/MiniMax-M2.5
```
...
```bash
docker run \
  -e AIM_PROFILE_ID=vllm-mi325x-fp8-tp2-latency \
  --device=/dev/kfd --device=/dev/dri \
  -p 8000:8000 \
  amdenterpriseai/aim-minimaxai-minimax-m2-5:0.12.0
```
