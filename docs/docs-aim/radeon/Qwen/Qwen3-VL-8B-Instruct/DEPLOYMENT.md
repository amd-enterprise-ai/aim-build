...
This guide provides step-by-step instructions for deploying AMD Inference Microservice (AIM) container for
 Qwen/Qwen3-VL-8B-Instruct model in
...
docker run \
  ...
  -p 8000:8000 \
  amdenterpriseai/aim-radeon-qwen-qwen3-vl-8b-instruct:0.12.0-preview
```
...
docker run --rm \
  ...
  amdenterpriseai/aim-radeon-qwen-qwen3-vl-8b-instruct:0.12.0-preview \
  download-to-cache --model-id Qwen/Qwen3-VL-8B-Instruct
```
...
spec:
  ...
  template:
    ...
    spec:
      containers:
        - name: minimal-aim-deployment
          image: amdenterpriseai/aim-radeon-qwen-qwen3-vl-8b-instruct:0.12.0-preview
...
aim_id: Qwen/Qwen3-VL-8B-Instruct
...
```bash
docker run \
  -e AIM_PROFILE_ID=vllm-r9700-bf16-tp1-latency \
  --device=/dev/kfd --device=/dev/dri \
  -p 8000:8000 \
  amdenterpriseai/aim-radeon-qwen-qwen3-vl-8b-instruct:0.12.0-preview
```
