...
This guide provides step-by-step instructions for deploying AMD Inference Microservice (AIM) container for
 meta-llama/Llama-3.2-1B-Instruct model in
...
docker run \
  ...
  -p 8000:8000 \
  amdenterpriseai/aim-epyc-meta-llama-llama-3-2-1b-instruct:0.11.0-preview
```
...
docker run --rm \
  ...
  amdenterpriseai/aim-epyc-meta-llama-llama-3-2-1b-instruct:0.11.0-preview \
  download-to-cache --model-id meta-llama/Llama-3.2-1B-Instruct
```
