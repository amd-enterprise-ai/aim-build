...
This guide provides step-by-step instructions for deploying AMD Inference Microservice (AIM) container for
 ibm-granite/granite-4.1-8b model in
...
docker run \
  -p 8000:8000 \
  amdenterpriseai/aim-epyc-ibm-granite-granite-4-1-8b:0.13.0
```
...
docker run --rm \
  ...
  amdenterpriseai/aim-epyc-ibm-granite-granite-4-1-8b:0.13.0 \
  download-to-cache --model-id ibm-granite/granite-4.1-8b
```
...
spec:
  ...
  template:
    ...
    spec:
      containers:
        - name: minimal-aim-deployment
          image: amdenterpriseai/aim-epyc-ibm-granite-granite-4-1-8b:0.13.0
...
aim_id: ibm-granite/granite-4.1-8b
